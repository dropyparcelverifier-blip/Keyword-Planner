...
try {
  ...
  # Launch through a wscript.exe shim so nothing flashes on screen.
  #
  # -WindowStyle Hidden does NOT prevent the flash: Windows creates the
  # console host window before powershell.exe parses that switch, so an
  # interactive task pops a black window every run. At a 2-minute interval
  # that is ~720 flashes a day on a machine somebody is trying to use.
  #
  # The usual advice — "run whether the user is logged on or not" — is WRONG
  # here. That puts the task in session 0, where the Start-Process chrome.exe
  # at the end of the watchdog would launch an invisible, unusable Chrome and
  # defeat the entire point of the watchdog. The task must stay interactive.
  #
  # wscript.exe has no console of its own, so Run(..., 0, False) starts
  # PowerShell fully hidden with no host window to flash. This lives in the
  # INSTALLER rather than being patched onto the task by hand, because the
  # installer unregisters and recreates the task on every run and would
  # otherwise silently undo the fix.
  $shimPath = Join-Path $root 'chrome-watchdog-hidden.vbs'
  $shimBody = @"
' Launches the AdBrain watchdog with no console window. Generated by the
' installer — see the comment in the installer for why wscript is required.
Set sh = CreateObject("WScript.Shell")
sh.Run "powershell.exe -NoProfile -ExecutionPolicy Bypass -File ""$watchdogPath""", 0, False
"@
  Set-Content -Path $shimPath -Value $shimBody -Encoding ASCII

  $taskName = 'AdBrain Chrome Watchdog'
  Unregister-ScheduledTask -TaskName $taskName -Confirm:$false -ErrorAction SilentlyContinue | Out-Null
  $tAction    = New-ScheduledTaskAction    -Execute 'wscript.exe' -Argument ('"' + $shimPath + '"')
  ...
  Write-Host '[AdBrain] Chrome watchdog installed - checks every 2 min, runs hidden (no console flash)' -ForegroundColor Green
} catch {
