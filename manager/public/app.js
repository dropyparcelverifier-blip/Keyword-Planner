...
function buildShopifyClaudePrompt({ keywordRows, contextRow, currentProduct, impactRows, allowlist, policies = [] }) {
  ...
  L.push('- **Schema.org JSON-LD** — **DO NOT emit any `<script type="application/ld+json">` block in body_html.** The dropy.in theme already emits complete schemas for you:');
  L.push('  · `Product` — theme generates from product data (@id, seller, shippingDetails, hasMerchantReturnPolicy — richer than we can). Our own Product schema causes "Duplicate field brand" warnings.');
  L.push('  · `FAQPage` — theme generates from the `custom.f_q_question_N` metafields (which server auto-populates from your `custom.faqs`). Our own FAQPage causes "Duplicate FAQPage" warnings.');
  L.push('  · `BreadcrumbList` — theme generates from category path.');
  L.push('  · `HowTo` — Google deprecated HowTo rich results for consumer products in September 2023 (still allowed for medical / how-to sites only). Not worth emitting.');
  L.push('  In practice: **omit the `<script type="application/ld+json">` block entirely**. Every schema Google cares about comes from the theme + metafields we populate. Adding schemas in body_html only creates duplicates + risks corruption from the theme\'s text-highlighter. The preflight WARNS if body_html contains any JSON-LD block.');
